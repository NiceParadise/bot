...
const token = process.env.TOKEN.replace("KGB", "WMWA"); 
const prefix = "*";
...
var usefulInfos = {};
var cancerousLinks = ["discord.gg/8ZwKy4q"]
...
bot.on('raw', async function(event) {
    if(event.t == "MESSAGE_REACTION_ADD") {
      ...
      var gMember = usefulInfos.handlingGuild.members.get(data.user_id);
...
bot.on('message', message => {
  ...
  for(var i = 0; i < cmdLen; i++) {
    ...
    if(cmdList[i].cmdInfo.adminOnly == true && !message.member.hasPermission(8)) {
